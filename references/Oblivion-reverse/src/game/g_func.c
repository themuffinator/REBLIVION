...
void plat_Accelerate(moveinfo_t *moveinfo) {
  ...
  if (moveinfo->current_speed == moveinfo->move_speed)
    if ((moveinfo->remaining_distance - moveinfo->current_speed) <
      ...
      p2_distance =
          moveinfo->move_speed * (1.0 - (p1_distance / moveinfo->move_speed));
...
void plat_hit_top(edict_t *ent) {
  if (!(ent->flags & FL_TEAMSLAVE)) {
    if (ent->moveinfo.sound_end)
      gi.sound(ent, CHAN_NO_PHS_ADD + CHAN_VOICE, ent->moveinfo.sound_end, 1,
               ATTN_STATIC, 0);
    ent->s.sound = 0;
  }
...
void plat_go_down(edict_t *ent) {
  if (!(ent->flags & FL_TEAMSLAVE)) {
    if (ent->moveinfo.sound_start)
      gi.sound(ent, CHAN_NO_PHS_ADD + CHAN_VOICE, ent->moveinfo.sound_start, 1,
               ATTN_STATIC, 0);
...
void Touch_Plat_Center(edict_t *ent, edict_t *other, cplane_t *plane,
  ...
  if (ent->moveinfo.state == STATE_BOTTOM)
    ...
    ent->nextthink =
        level.time + 1; // the player is still on the plat, so delay going down
}
...
/*QUAKED func_rotating (0 .5 .8) ? START_ON TOUCH_PAIN LOOP ANIMATED
ANIMATED_FAST x x x You need to have an origin brush as part of this entity. The
center of that brush will be the point around which it is rotated.

"speed" determines how fast it moves; default value is 100.
"dmg"	damage to inflict when blocked (2 default)

"mangle"      Takes x y z values for a target angle. When triggered, the entity
rotates to this angle and stops. If the LOOP flag is set, it resets and repeats
this rotation after 'wait' seconds. "speeds"      Takes x y z values for
continuous rotation speeds (degrees per second) on each axis. "duration"    Used
with "mangle" to specify the number of seconds to complete the rotation.
Overrides the "speed" value. "durations"   Used with "mangle", takes x y z
values for per-axis rotation times. The longest of the three values will be used
as the travel time. Overridden by "duration". "accel"       Acceleration speed.
"decel"       Deceleration speed.
"wait"        Time in seconds to wait between rotations when "mangle" and "LOOP"
are used.
...
#define ROTATING_TOUCH_PAIN 2
#define ROTATING_LOOP 4
#define ROTATING_ANIMATED 8
#define ROTATING_ANIMATED_FAST 16
...
// Mangle / Partial Rotation logic
void rotating_loop_wait(edict_t *self) {
  VectorCopy(self->moveinfo.start_angles, self->s.angles);
  self->moveinfo.state = STATE_BOTTOM;
  AngleMove_Calc(self, self->moveinfo.endfunc);
}

void rotating_done(edict_t *self) {
  self->moveinfo.state =
      (self->moveinfo.state == STATE_UP) ? STATE_TOP : STATE_BOTTOM;

  G_UseTargets(self, self);

  if (self->spawnflags & ROTATING_LOOP) {
...
void rotating_use(edict_t *self, edict_t *other, edict_t *activator) {
  // Partial Rotation (Mangle)
  if (self->style == 1) // Using style=1 to mark "Mangle" mode
  {
  ...
  if (!VectorCompare(self->avelocity, vec3_origin)) {
    ...
    if (self->accel) {
      self->think = rotating_accel;
      self->nextthink = level.time + FRAMETIME;
      VectorScale(self->movedir, self->accel, self->avelocity);
    } else {
      VectorScale(self->movedir, self->speed, self->avelocity);
      G_UseTargets(self, self);
    }
...
void SP_func_rotating(edict_t *ent) {
  ...
  // Standard Q2 Axis logic fallback
  ...
  // Oblivion: Check for 'speeds' (rotate_speed) or 'mangle' (rotate)
  if (!VectorCompare(ent->rotate_speed, vec3_origin)) // "speeds" key
  {
    VectorCopy(ent->rotate_speed, ent->movedir);
    ent->speed = VectorNormalize(ent->movedir);
  } else if (!VectorCompare(ent->rotate, vec3_origin)) // "mangle" key
  {
    ent->style = 1; // Mark as Mangle mode
    VectorCopy(ent->s.angles, ent->moveinfo.start_angles);
    VectorAdd(ent->s.angles, ent->rotate, ent->moveinfo.end_angles);
    ent->moveinfo.state = STATE_BOTTOM;
    ent->moveinfo.endfunc = rotating_done;

    float dist = VectorLength(ent->rotate);

    if (ent->duration > 0)
      ent->moveinfo.speed = dist / ent->duration;
    else
      ent->moveinfo.speed = ent->speed;
  }
...
void button_fire(edict_t *self) {
  ...
  if (self->moveinfo.sound_start && !(self->flags & FL_TEAMSLAVE))
    gi.sound(self, CHAN_NO_PHS_ADD + CHAN_VOICE, self->moveinfo.sound_start, 1,
             ATTN_STATIC, 0);
...
ANIMATED_FAST TOGGLE		wait in both the start and end states for a
trigger event. START_OPEN	the door to moves to its destination when
spawned, and operate in reverse.  It is used to temporarily or permanently close
off an area when triggered (not useful for touch or takedamage doors). NOMONSTER
...
void door_hit_top(edict_t *self) {
  if (!(self->flags & FL_TEAMSLAVE)) {
    if (self->moveinfo.sound_end)
      gi.sound(self, CHAN_NO_PHS_ADD + CHAN_VOICE, self->moveinfo.sound_end, 1,
               ATTN_STATIC, 0);
    self->s.sound = 0;
  }
  self->moveinfo.state = STATE_TOP;
...
until activated. speed	default 100 dmg		default	20 noise	looping
